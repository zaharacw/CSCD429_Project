...
dailyDiffs <- function(x) {
  ...
  for (i in 1:(dim(x)[1] - 1)) {
    ...
    x$CloseDiffPercent[i] <- x$CloseDiff[i] / x$Close[i+1]
...
stock_labels_chron <- stock_labels[seq(dim(stock_labels)[1],1),]
write.csv(stock_labels_chron, saveFile, row.names = FALSE)
```